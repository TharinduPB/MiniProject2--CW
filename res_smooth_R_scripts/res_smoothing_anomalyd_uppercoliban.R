...
res_data1 <- read_csv("reslevels_raw.csv") %>% 
  ...
  arrange(timestamp) %>% 
  dplyr::filter( !is.na(level) & level !=0 & reservoir == "Upper Coliban" &
...
res_data_1 <- phycon(res_data,507, 490)

res_data_2 <- mad_outliers(res_data_1, 3.5)

res_data_3 <- sgfilter(res_data_2,0,11,0.03)
...
plot_ly() %>%
  ...
  layout(title = 'upper coliban original vs smooth',
